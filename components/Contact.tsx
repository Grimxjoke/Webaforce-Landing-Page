      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 flex flex-col lg:flex-row min-h-[600px] transition-colors duration-300">
          ...
          <div className="lg:w-5/12 relative flex flex-col justify-between p-10 text-white overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-100 mb-8 text-lg leading-relaxed">
                Obtenez une analyse réelle de votre visibilité par rapport à vos concurrents locaux. C'est gratuit et sans engagement.
              </p>
              ...
              <div className="space-y-8 mt-12">
                ...
                <div className="flex items-center group">
                  ...
                  <span className="text-lg font-bold">Rapport PDF sous 24h</span>
                </div>
              </div>
            </div>
          ...
          <div className="lg:w-7/12 p-10 bg-white dark:bg-slate-800">
            <div className="mb-8">
                <h4 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Recevoir mon audit gratuit</h4>
                <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 uppercase font-bold">Vérification de votre fiche Google Maps et de votre page web.</p>
            </div>
            ...
            {isSubmitted ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-500/30 rounded-lg">
                ...
                <p className="text-slate-600 dark:text-slate-300">Nous préparons votre audit de visibilité. Un expert vous contactera d'ici demain.</p>
              </div>
            ) : (
              <form className="space-y-6" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                      ...
                      <input type="text" id="name" required value={formData.name} onChange={handleChange} className="block w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 focus:ring-2 focus:ring-accent-600 outline-none transition-all font-bold" placeholder="Jean Dupont" />
                  </div>
                  <div>
                      ...
                      <input type="text" id="company" required value={formData.company} onChange={handleChange} className="block w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 focus:ring-2 focus:ring-accent-600 outline-none transition-all font-bold" placeholder="Toiture Pro 75" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                      ...
                      <input type="tel" id="phone" required value={formData.phone} onChange={handleChange} className="block w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 focus:ring-2 focus:ring-accent-600 outline-none transition-all font-bold" placeholder="06 12 34 56 78" />
                      {errors.phone && <p className="mt-1 text-xs text-red-500 font-bold">{errors.phone}</p>}
                  </div>
                  <div>
                      <label htmlFor="email" className="block text-xs font-black text-slate-700 dark:text-slate-300 mb-2 uppercase tracking-widest">Email professionnel</label>
                      <input type="email" id="email" required value={formData.email} onChange={handleChange} className="block w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 focus:ring-2 focus:ring-accent-600 outline-none transition-all font-bold" placeholder="jean@toiturepro.fr" />
                ...
                <div className="flex items-center gap-3 p-4 bg-accent-600/5 rounded-lg border border-accent-600/20">
                    ...
                    <label htmlFor="auditRequested" className="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight cursor-pointer select-none">Je souhaite recevoir mon audit Google Maps + Web gratuit</label>
                </div>
